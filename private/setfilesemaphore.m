...
%
%		Note: A semaphore older than 20 seconds (GOKBERK: Increased this to 2 minutes!) is considered as
%		invalid and will immediately be deleted.
%
%		Example:
...
%semaphoreOldTime = 20; % original
semaphoreOldTime = 120; % GOKBERK
...
for fileNr = 1:nOfFiles
    ...
    while 1
        ...
        if ~isempty(dirStruct)
            ...
            % display info
            if semaphoreExisting  && (now - startWaitTime) * 86400 >= waitInfoPeriod
                % GOKBERK
                if displayWaitInfo
                    ...
                    fprintf('.'); % show trials..
                    gokberk_counter  = gokberk_counter + 1;

                    if mod(gokberk_counter,10) && isunix
                        % GOKBERK Force NSF update
                        % Otherwise we see the files with types as ??????
                        disp('gokberk:force NSF update');
                        gokberk_foo = fullfile(fileparts(fileName),'foo');
                        cn_system(['rm -f ' gokberk_foo]);
                        cn_system(['echo x > ' gokberk_foo]);
                        % todo: ls -la on the directory is NOT ok?
                    end
                end
            end
...
% GOKBERK: DONT CHANGE THE RANDOM STATE OF MATLAB

persistent generator

org_randinfo = cn_saverandstream;

if isempty(generator)
    generator = RandStream('mt19937ar','seed',sum(100*clock));
end
%RandStream.setDefaultStream(generator);
RandStream.setGlobalStream(generator);
...
% GOKBERK PRESERVE THE ORIGINAL RAND STATE BEFORE CALLING THIS FUNCTION
cn_loadrandstream(org_randinfo);
